...
class FixedWindow(Windowing):
    ...
    def initialize_controls(self, initial_expression=Constant(0.0)):
        ...
        """
        if (
            not isinstance(initial_expression, list)
            and self.pde_solver.num_controls > 1
        ):
            initial_expression = [initial_expression]

        if (
            self.pde_solver.num_controls > 1
            and len(initial_expression) != self.pde_solver.num_controls
        ):
            raise ValueError(
                f"Length of initial_expression list ({len(initial_expression)}) must match the number of controls ({self.pde_solver.num_controls})."
            )

        if self.pde_solver.num_controls == 1:
            self.window_controls = [
                Function(self.pde_solver.V, name=f"Control at time-hop{i}")
            ...
            self.window_controls = [
                [
                    Function(self.pde_solver.V, name=f"Control{j} at time-hop{i}")
                    for i in range(self.window_size)
                ]
