...
def test_heat_equation_solver_set_forcing_function():
    ...
    assert np.allclose(
        heat_solver.f.dat.data, f_new.dat.data
    ), "Forcing function not set correctly."
    ...
    assert np.allclose(
        heat_solver.f.dat.data, f_expr_func.dat.data
    ), "Forcing function not set correctly with expression."
...
def test_heat_equation_solver_set_initial_condition():
    ...
    assert np.allclose(
        heat_solver.u0.dat.data, u0_new.dat.data
    ), "Initial condition not set correctly."
    ...
    assert np.allclose(
        heat_solver.u0.dat.data, u0_expr_func.dat.data
    ), "Initial condition not set correctly with expression."
...
def test_heat_equation_set_bcs():
    ...
    assert all(
        isinstance(bc, DirichletBC) for bc in heat_solver.bcs
    ), "Boundary conditions are not DirichletBC instances."
...
def test_heat_equation_solver_build_and_solve():
    ...
    heat_solver.solve()
    assert isinstance(
        heat_solver.u_new, Function
    ), "Solution not a Function instance after solving."
